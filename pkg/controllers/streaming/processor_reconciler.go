...
const (
	ProcessorImagesStashKey reconcilers.StashKey = "processor-images"
	InputStreamsStashKey    reconcilers.StashKey = "input-streams"
	OutputStreamsStashKey   reconcilers.StashKey = "output-streams"
)
...
func ProcessorReconciler(c reconcilers.Config, namespace string) *reconcilers.ParentReconciler {
...
func ProcessorSyncProcessorImages(c reconcilers.Config, namespace string) reconcilers.SubReconciler {
	...
	return &reconcilers.SyncReconciler{
		Sync: func(ctx context.Context, processor *streamingv1alpha1.Processor) error {
			...
			reconcilers.StashValue(ctx, ProcessorImagesStashKey, config.Data)
			return nil
		},
...
func ProcessorResolveStreamsReconciler(c reconcilers.Config) reconcilers.SubReconciler {
	...
	return &reconcilers.SyncReconciler{
		Sync: func(ctx context.Context, processor *streamingv1alpha1.Processor) error {
			...
			reconcilers.StashValue(ctx, InputStreamsStashKey, inputStreams)
			...
			reconcilers.StashValue(ctx, OutputStreamsStashKey, outputStreams)
...
func ProcessorChildDeploymentReconciler(c reconcilers.Config) reconcilers.SubReconciler {
	...
	return &reconcilers.ChildReconciler{
		...
		DesiredChild: func(ctx context.Context, parent *streamingv1alpha1.Processor) (*appsv1.Deployment, error) {
			...
			outputStreams, ok := reconcilers.RetrieveValue(ctx, OutputStreamsStashKey).([]streamingv1alpha1.Stream)
			if !ok {
				return nil, nil
			}
			processorImages, ok := reconcilers.RetrieveValue(ctx, ProcessorImagesStashKey).(map[string]string)
			if !ok {
				return nil, nil
			}
...
func ProcessorChildScaledObjectReconciler(c reconcilers.Config) reconcilers.SubReconciler {
