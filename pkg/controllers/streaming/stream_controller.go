...
func (r *StreamReconciler) reconcile(ctx context.Context, log logr.Logger, stream *streamingv1alpha1.Stream) (ctrl.Result, error) {
	...
	if childBindingMetadata == nil {
		stream.Status.BindingRef.MetadataRef = corev1.ObjectReference{}
	} else {
		stream.Status.BindingRef.MetadataRef = corev1.ObjectReference{
			APIVersion: "v1",
			Kind:       "ConfigMap",
			Namespace:  childBindingMetadata.Namespace,
			Name:       childBindingMetadata.Name,
		}
	}
	...
	if childBindingSecret == nil {
		stream.Status.BindingRef.SecretRef = corev1.ObjectReference{}
	} else {
		stream.Status.BindingRef.SecretRef = corev1.ObjectReference{
			APIVersion: "v1",
			Kind:       "Secret",
			Namespace:  childBindingSecret.Namespace,
			Name:       childBindingSecret.Name,
		}
	}
...
func (r *StreamReconciler) reconcileChildBindingSecret(ctx context.Context, log logr.Logger, stream *streamingv1alpha1.Stream) (*corev1.Secret, error) {
	...
	bindingSecret.Data = desiredBindingSecret.Data
